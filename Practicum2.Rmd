...
## Extracting the Elocation table from the XML
```{r}
node <- xpathApply(root, "//Article")
e_id <- xmlSApply(node, function(x)xmlValue(x[['ELocationID']]))
e_
xpath <- "//Article/ELocationID[@EIdType]"
e_type <- xpathSApply(root,xpath,xmlGetAttr,"EIdType")

Elocation <- data.frame(e_id = e_id,
                        e_type = e_type,
                        stringsAsFactors = F)
Elocation
